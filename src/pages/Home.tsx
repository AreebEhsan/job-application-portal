        ...
        <div className="w-full max-w-5xl mx-auto text-center space-y-10">
          <div className="glass mx-auto max-w-3xl px-8 md:px-12 py-10 md:py-14 space-y-6 fade-in">
            ...
            <p className="text-base md:text-lg lg:text-xl text-white/80 max-w-2xl mx-auto">
              Browse, apply, and track your applications — whether you're job hunting or hiring.
            </p>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 py-12 md:py-14">
        <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-white/70 text-sm mt-1">Fresh roles from top companies.</p>
          </div>
          <a href="/jobs" className="text-emerald-300 hover:text-emerald-200 text-sm whitespace-nowrap">View all</a>
        </header>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {[
            { company: 'DataForge Labs', title: 'Frontend Engineer', desc: 'Own the UI and DX. Work with React, TypeScript, Tailwind.' },
            { company: 'Orbital Systems', title: 'Data Analyst', desc: 'Dashboards + SQL queries.' },
            { company: 'GreenPulse', title: 'Cloud Engineer', desc: 'Infra as code + CI/CD.' },
          ].map(job => (
            <div key={job.title} className="glass p-5">
              <div className="text-sm text-white/70">{job.company}</div>
              <h3 className="text-lg md:text-xl font-semibold mt-1">{job.title}</h3>
              <p className="text-white/80 mt-2 line-clamp-2">{job.desc}</p>
